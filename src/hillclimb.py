...
# A slightly modified version of `pgmpy.estimators.HillClimbSearch`.
...
class HillClimbSearchAll(StructureEstimator):
    def __init__(self, data, use_cache=True, **kwargs):
        ...
        super(HillClimbSearchAll, self).__init__(data, **kwargs)
    ...
    def hill_path(
        ...
        Estimated model: pgmpy.base.DAG
            A `DAG` somewhere along the path up the hill.
        ...
        if not hasattr(fixed_edges, "__iter__"):
            ...
            if not nx.is_directed_acyclic_graph(start_dag):
                raise ValueError(
                    "fixed_edges creates a cycle in start_dag. Please modify \
                    either fixed_edges or start_dag."
                )
